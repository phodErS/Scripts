...
local cubeNames = {"Luck Cube","Speed Cube","Glitch Cube","Haste Cube","Fortune Cube","Pastel Cube","Golden Cube","Inferno Cube","Wealth Cube","Multitude of Rain","Fortune of Wind","Silent Speed of Snow","Spore Blossom","Technosphere","Event Horizon","Burger","Arctic Frost","Dragon`s Gem","Midas Touch","Lumitree","Shooting Star","Glitch Anomaly","Roll of Glitch","Stellar Nebula"};
local cubeRarities = {["Luck Cube"]="Common",["Speed Cube"]="Common",["Multitude of Rain"]="Common",["Fortune of Wind"]="Common",["Silent Speed of Snow"]="Common",["Haste Cube"]="Uncommon",["Fortune Cube"]="Uncommon",["Wealth Cube"]="Rare",["Pastel Cube"]="Epic",["Golden Cube"]="Epic",["Glitch Cube"]="Legendary",["Inferno Cube"]="Legendary",["Spore Blossom"]="Mythic",Technosphere="Mythic"};
local cubeColors = {["Luck Cube"]="#05ff3b",["Speed Cube"]="#6397ff",["Inferno Cube"]="#fc6330",["Glitch Cube"]="#440345",["Haste Cube"]="#0558ff",["Fortune Cube"]="#288000",["Pastel Cube"]="#cc96ff",["Golden Cube"]="#fab546",["Wealth Cube"]="#ffec59",["Spore Blossom"]="#ed4aff",Technosphere="#5c4aff",["Event Horizon"]="#6854ff",["Fortune of Wind"]="#8cf19b",["Silent Speed of Snow"]="#acecff",["Multitude of Rain"]="#3bb8ff",["Stellar Nebula"]="#ffffff"};
...
local function sendDiscordNotification(cubeName)
	...
	local Embed = {title="Tycoon RNG Notification",description=(mention .. " A cube has been collected in the game!"),color=RGBToDecimal(r, g, b),footer={text=("Job ID: " .. game.JobId)},fields={{name="Cube Name",value=("`" .. cubeName .. "`"),inline=true},{name="Status",value="`Collected`",inline=true},{name="Rarity",value=("`" .. rarity .. "`"),inline=false}},timestamp=string.format("%d-%d-%dT%02d:%02d:%02dZ", Time.year, Time.month, Time.day, Time.hour, Time.min, Time.sec)};
	local Body = {content=mention,embeds={Embed}};
	((syn and syn.request) or http_request)({Url=webhookUrl,Method="POST",Headers={["Content-Type"]="application/json"},Body=game:GetService("HttpService"):JSONEncode(Body)});
end
local function sendObbyEndNotification()
	local OSTime = os.time();
	local Time = os.date("!*t", OSTime);
	local mention = (pingme and "@everyone") or "";
	local Embed = {title="Tycoon RNG Notificationt",description="The player has been successfully teleported to the end of the Luck Obby.",color=RGBToDecimal(144, 214, 254),footer={text=game.JobId},fields={{name="Luck Obby Status",value="`Completed`"}},timestamp=string.format("%d-%d-%dT%02d:%02d:%02dZ", Time.year, Time.month, Time.day, Time.hour, Time.min, Time.sec)};
	local Body = {content=mention,embeds={Embed}};
	((syn and syn.request) or http_request)({Url=webhookUrl,Method="POST",Headers={["Content-Type"]="application/json"},Body=game:GetService("HttpService"):JSONEncode(Body)});
end
...
local ModuleHandler = (function()
	local ModuleHandler = {};
	ModuleHandler.include = function(self, ModuleName)
		if not SYC then
			return;
		end
		if not SYC.Modules then
			return;
		end
	...
	getgenv().include = function(modname)
		return ModuleHandler:include(modname);
	end;
	return ModuleHandler;
end)();
do
	do
		SYC.Modules.Connect = function(onething, secondthing)
	...
	do
		SYC.Modules.PerlinNoise = function(offset, speed, time)
			local value = math.noise((time * speed) + offset);
			return math.clamp(value, -0.5, 0.5);
		end;
	end
	do
		SYC.Modules.UI.RichTextToNormalText = function(str)
			local output_string = str;
			while true do
				if (not output_string:find("<") and not output_string:find(">")) then
					break;
				end
				if ((output_string:find("<") and not output_string:find(">")) or (output_string:find(">") and not output_string:find("<"))) then
					return error("Invalid RichText");
				end
	...
	do
		SYC.Modules.UI.GetTextBoundary = function(self, Text, Font, Size, Resolution)
...
local UserInterface = (function()
	local UserInterface = {Instances={},Popup=nil,KeybindsListObjects={},KeybindList=nil,Flags={},ConfigFlags={}};
	getgenv().uishit = UserInterface;
	getgenv().theme = {accent=Color3FromRGB(167, 140, 255)};
	getgenv().theme_event = Instance.new("BindableEvent");
	getgenv().UI = UserInterface.Instances;
	local UIModule = include("UI");
	local dragging, dragInput, dragStart, startPos, dragObject;
	local Keys = {[Enum.KeyCode.LeftShift]="LS",[Enum.KeyCode.RightShift]="RS",[Enum.KeyCode.LeftControl]="LC",[Enum.KeyCode.RightControl]="RC",[Enum.KeyCode.LeftAlt]="LA",[Enum.KeyCode.RightAlt]="RA",[Enum.KeyCode.CapsLock]="CAPS",[Enum.KeyCode.Return]="ENT",[Enum.KeyCode.PageDown]="PGD",[Enum.KeyCode.PageUp]="PGU",[Enum.KeyCode.ScrollLock]="SCL",[Enum.KeyCode.One]="1",[Enum.KeyCode.Two]="2",[Enum.KeyCode.Three]="3",[Enum.KeyCode.Four]="4",[Enum.KeyCode.Five]="5",[Enum.KeyCode.Six]="6",[Enum.KeyCode.Seven]="7",[Enum.KeyCode.Eight]="8",[Enum.KeyCode.Nine]="9",[Enum.KeyCode.Zero]="0",[Enum.KeyCode.KeypadOne]="1",[Enum.KeyCode.KeypadTwo]="2",[Enum.KeyCode.KeypadThree]="3",[Enum.KeyCode.KeypadFour]="4",[Enum.KeyCode.KeypadFive]="5",[Enum.KeyCode.KeypadSix]="6",[Enum.KeyCode.KeypadSeven]="7",[Enum.KeyCode.KeypadEight]="8",[Enum.KeyCode.KeypadNine]="9",[Enum.KeyCode.KeypadZero]="0",[Enum.KeyCode.Minus]="-",[Enum.KeyCode.Equals]="=",[Enum.KeyCode.Tilde]="~",[Enum.KeyCode.LeftBracket]="[",[Enum.KeyCode.RightBracket]="]",[Enum.KeyCode.RightParenthesis]=")",[Enum.KeyCode.LeftParenthesis]="(",[Enum.KeyCode.Semicolon]=",",[Enum.KeyCode.Quote]="'",[Enum.KeyCode.BackSlash]="\\",[Enum.KeyCode.Comma]=",",[Enum.KeyCode.Period]=".",[Enum.KeyCode.Slash]="/",[Enum.KeyCode.Asterisk]="*",[Enum.KeyCode.Plus]="+",[Enum.KeyCode.Period]=".",[Enum.KeyCode.Backquote]="`",[Enum.KeyCode.Insert]="INS",[Enum.UserInputType.MouseButton1]="MB1",[Enum.UserInputType.MouseButton2]="MB2",[Enum.UserInputType.MouseButton3]="MB3",[Enum.KeyCode.Backspace]="BS",[Enum.KeyCode.Escape]="ESC",[Enum.KeyCode.Space]="SPC"};
	local FlagCount = 0;
	UserInterface.GetNextFlag = function(self)
		FlagCount = FlagCount + 1;
		return tostring(FlagCount);
	end;
	UserInterface.Create = function(self, OptionsLaughtOutLouds)
		local Configuration = {Tabs={},Title=(OptionsLaughtOutLouds.title or 'syndicate<font color="rgb(129, 127, 127)">.club</font>')};
		local Texts = {"Tycoon RNG Script"};
		...
		Configuration.Tab = function(self, Tab_Name)
			if (not type(Tab_Name) == "string") then
				return;
			end
			...
			TabConfiguration.Select = function(self)
			...
			TabConfiguration.Section = function(self, Section_Name, Side)
				if (not type(Section_Name) == "string") then
					return;
				end
				if (not type(Side) == "string") then
					return;
				end
				...
				do
					do
						Options.BoneSelector = function(self, Configuration)
							local BoneSelectorOptions = {Type=(Configuration.type or "R15"),Callback=(Configuration.callback or function()
							end),Default=(Configuration.default or nil),Flag=UserInterface:GetNextFlag(),Multi=(Configuration.multi or false)};
							local BoneSelector = {FValues={},FValue=((BoneSelectorOptions.Multi and {}) or "")};
							...
							BoneSelectorHolder.Position = UDim2.new(0, 0, -0.000001173753, 0);
							...
							local function R15_function()
								for _, bodypart in pairs(R15:GetChildren()) do
									...
									BoneSelector['FValues'][name] = {Click=ButtonClick,UnClick=ButtonUnClick,Selected=false};
								end
							end
							...
							R15_function();
							BoneSelectorOptions.Update = function(self)
							...
							BoneSelectorOptions.Set = function(self, value)
								if BoneSelectorOptions.Multi then
									if (type(value) == "table") then
										...
										end,[2]=function(value)
											BoneSelectorOptions:Set(value);
										end};
										return;
									end
							...
							BoneSelectorOptions.GetValues = function(self)
								return BoneSelector.FValue;
							end;
							BoneSelectorOptions.Refresh = function(self)
							...
							BoneSelectorOptions.SetMulti = function(self, bool)
								if (BoneSelectorOptions.Multi == bool) then
									return;
								end
								self:Refresh();
							...
							UserInterface.ConfigFlags[BoneSelectorOptions.Flag] = function(state)
								BoneSelectorOptions:Set(state);
							end;
							increaseYSize(308);
					...
					do
						Options.Button = function(self, Configuration)
							local ButtonOptions = {title=(Configuration.title or "button"),callback=(Configuration.callback or function()
							end)};
							...
							local Tweens = {OnClick=function()
								local ButtonTween, ButtonStrokeTween = tweenService:Create(Button, TweenInfo.new(0), {BackgroundColor3=Color3FromRGB(36, 36, 36)}), tweenService:Create(ButtonStroke, TweenInfo.new(0), {Color=Color3FromRGB(45, 43, 46)});
								ButtonTween:Play();
								ButtonStrokeTween:Play();
							end,OnHover=function()
								local ButtonTween, ButtonStrokeTween = tweenService:Create(Button, TweenInfo.new(0), {BackgroundColor3=Color3FromRGB(28, 28, 28)}), tweenService:Create(ButtonStroke, TweenInfo.new(0), {Color=Color3FromRGB(36, 36, 36)});
								ButtonTween:Play();
								ButtonStrokeTween:Play();
							end,OnMouseLeave=function()
								local ButtonTween, ButtonStrokeTween = tweenService:Create(Button, TweenInfo.new(0), {BackgroundColor3=Color3FromRGB(21, 21, 21)}), tweenService:Create(ButtonStroke, TweenInfo.new(0), {Color=Color3FromRGB(40, 40, 40)});
								ButtonTween:Play();
								ButtonStrokeTween:Play();
							end};
					...
					do
						Options.Colorpicker = function(self, Configuration, ToggleOption)
							local ColorpickerOptions = {Title=(Configuration.title or "colorpicker"),Default=(Configuration.default or Color3FromRGB(255, 255, 255)),Transparency=(Configuration.transparency or 0),Callback=(Configuration.callback or function()
							end),Flag=UserInterface:GetNextFlag()};
							local Colorpicker = {TransparencyValue=0,ColorValue=nil,HuePosition=0,SlidingSat=false,SlidingHue=false,SlidingAlpha=false};
							...
							CPHueGradient.Color = ColorSequence.new({ColorSequenceKeypoint.new(0, Color3FromRGB(255, 0, 0)),ColorSequenceKeypoint.new(0.17, Color3FromRGB(255, 0, 255)),ColorSequenceKeypoint.new(0.33, Color3FromRGB(0, 0, 255)),ColorSequenceKeypoint.new(0.5, Color3FromRGB(0, 255, 255)),ColorSequenceKeypoint.new(0.67, Color3FromRGB(0, 255, 0)),ColorSequenceKeypoint.new(0.83, Color3FromRGB(255, 255, 0)),ColorSequenceKeypoint.new(1, Color3FromRGB(255, 0, 0))});
							...
							TransparencyGradient.Color = ColorSequence.new({ColorSequenceKeypoint.new(0, Color3FromRGB(255, 255, 255)),ColorSequenceKeypoint.new(0.57, Color3FromRGB(150, 150, 150)),ColorSequenceKeypoint.new(1, Color3FromRGB(0, 0, 0))});
							...
							TransparencyPicker.BackgroundTransparency = 1;
							...
							local contentAnimations = {Open=function(self)
							...
							contentAnimations.Close = function(self)
							...
							ColorpickerOptions.Set = function(self, color, trans, ignore)
							...
							increaseYSize(13);
							UserInterface.ConfigFlags[ColorpickerOptions.Flag] = function(value)
								ColorpickerOptions:Set(value);
							end;
							return ColorpickerOptions;
						end;
					end
					do
						Options.Dropdown = function(self, Configuration)
							local DropdownOptions = {Title=(Configuration.title or ""),Content=(Configuration.values or {}),Default=(Configuration.default or "-"),Multi=(Configuration.multi or false),Callback=(Configuration.callback or function()
							end),Flag=UserInterface:GetNextFlag()};
							local Dropdown = {FValues={},FValue=((DropdownOptions.Multi and {}) or "")};
							...
							local Count = 0;
							Dropdown.CreateValue = function(self, name)
								if not Dropdown.FValues[name] then
									...
									Count += 1
									Dropdown.FValues[name] = {Click=Click,Unclick=Unclick,Objects=Objects,Selected=false};
									theme_event.Event:Connect(function()
							...
							Dropdown.Set = function(self, value)
								if DropdownOptions.Multi then
									if (typeof(value) == "table") then
										...
										UserInterface.Flags[DropdownOptions.Flag .. "f"] = {[1]=function(value)
							...
							Dropdown.Refresh = function(self, tbl)
								for _, v in pairs(Dropdown.FValues) do
							...
							UserInterface.ConfigFlags[DropdownOptions.Flag] = function(state)
								Dropdown:Set(state);
							end;
							increaseYSize(40);
					...
					do
						Options.Keybind = function(self, Configuration, toggle)
							local KeybindOptions = {Title=(Configuration.title or ""),Mode=(Configuration.mode or "Toggle"),Key=(Configuration.key or ""),Callback=(Configuration.callback or function()
							end),KeybindsList=(Configuration.keybindlist or false),KeybindListName=(Configuration.keybindname or self.Title),Flag=UserInterface:GetNextFlag()};
							local Keybind = {FMode=KeybindOptions.Mode,FKey=KeybindOptions.Key,Toggled=false,Picking=false,Modes={}};
							...
							KeybindInline.BackgroundTransparency = 1;
							...
							Keybind.UpdateList = function(self)
							...
							Keybind.Value = function(self, info)
								if info then
									if info[1] then
										local Key = info[1];
							...
							KeybindOptions.GetState = function(self)
							...
							inputService.InputBegan:Connect(function(input, gameprocessing)
								...
								if ((input.KeyCode.Name == Keybind.FKey) or (input.UserInputType.Name == mb)) then
									...
									elseif (Keybind.FMode == "Hold") then
										...
										local c;
										c = inputService.InputEnded:Connect(function(input)
											if ((input.KeyCode.Name == Keybind.FKey) or (input.UserInputType.Name == mb)) then
												c:Disconnect();
							...
							UserInterface.Flags[KeybindOptions.Flag] = KeybindOptions:GetState();
							UserInterface.Flags[KeybindOptions.Flag .. "_info"] = {Keybind.FKey,Keybind.FMode};
							UserInterface.ConfigFlags[KeybindOptions.Flag .. "_info"] = function(info)
								Keybind:Value(info);
							end;
					...
					do
						Options.Label = function(self, text, richtext)
							local LabelOptions = {};
							...
							LabelOptions.ChangeText = function(self, newtext)
								if (not type(newtext) == "string") then
									return;
								end
					...
					do
						Options.PlayerList = function(self)
							...
							PlayerlistHolder.Position = UDim2New(0, 0, -0.000001173753, 0);
							...
							local PlayerOptions = {CurrentPlayer=""};
							PlayerOptions.Clear = function(self)
							...
							PlayerOptions.Add = function(self, player, player_name)
							...
							PlayerOptions.Refresh = function(self)
								PlayerOptions:Clear();
							...
							PlayerOptions.GetPlayers = function(self)
								return Players:GetChildren();
							end;
							PlayerOptions.Search = function(self)
							...
							PlayerOptions.GetCurrentPlayer = function(self)
								return PlayerOptions.CurrentPlayer;
							end;
							PlayerOptions.Init = function(self)
					...
					do
						Options.Slider = function(self, Configuration)
							local SliderOptions = {Title=(Configuration.title or "slider"),Min=(Configuration.min or 1),Max=(Configuration.max or 10),Float=(Configuration.float or 1),Default=(Configuration.default or 0),Value=0,Callback=(Configuration.callback or function()
							end),Sliding=false,Suffix=(Configuration.suffix or ""),Flag=UserInterface:GetNextFlag()};
							...
							SliderOptions.Set = function(self, value)
							...
							SliderOptions.Slide = function(self, input)
							...
							UserInterface.ConfigFlags[SliderOptions.Flag] = function(value)
								SliderOptions:Set(value);
							end;
							increaseYSize(30);
					...
					do
						Options.TextBox = function(self, Configuration)
							local TextBoxOptions = {Title=(Configuration.title or "textbox"),Default=(Configuration.default or ""),Placeholder=(Configuration.placeholder or ""),ClearTextOnFocus=(Configuration.cleartextonfocus or Configuration.ctf or true),Callback=(Configuration.callback or function()
							end),Text="",Flag=UserInterface:GetNextFlag()};
							...
							TextBoxOptions.Set = function(self, Text)
								Text = Text or "";
								TextBox.Text = Text;
								TextBoxOptions.Text = TextBox.Text;
								UserInterface.Flags[TextBoxOptions.Flag] = TextBoxOptions.Text;
								UserInterface.Flags[TextBoxOptions.Flag .. "f"] = function(value)
									TextBoxOptions:Set(value);
								end;
							...
							increaseYSize(14);
							UserInterface.ConfigFlags[TextBoxOptions.Flag] = function(text)
								TextBoxOptions:Set(text);
							end;
							return TextBoxOptions;
						end;
					end
					do
						Options.Toggle = function(self, Configuration)
							local ToggleOptions = {title=(Configuration.title or "toggle"),default=(Configuration.default or false),state=false,callback=(Configuration.callback or function()
							end),Flag=UserInterface:GetNextFlag()};
							...
							local TS_ON = tweenService:Create(ToggleStatus, TweenInfo.new(0.2), {BackgroundColor3=Color3FromRGB(168, 157, 159)});
							local TN_ON = tweenService:Create(ToggleName, TweenInfo.new(0.2), {TextColor3=Color3FromRGB(255, 255, 255)});
							local TS_OFF = tweenService:Create(ToggleStatus, TweenInfo.new(0.2), {BackgroundColor3=Color3FromRGB(20, 20, 17)});
							local TN_OFF = tweenService:Create(ToggleName, TweenInfo.new(0.2), {TextColor3=Color3FromRGB(129, 129, 127)});
							theme_event.Event:Connect(function()
								TS_ON = tweenService:Create(ToggleStatus, TweenInfo.new(0.2), {BackgroundColor3=theme.accent});
							...
							ToggleOptions.Set = function(self, boolean)
							...
							ToggleOptions.Keybind = function(self, Configuration)
								Options:Keybind(Configuration, Button);
							end;
							ToggleOptions.Colorpicker = function(self, Configuration)
							...
							increaseYSize(15);
							UserInterface.ConfigFlags[ToggleOptions.Flag] = function(state)
								ToggleOptions:Set(state);
							end;
		...
		local function isMouseInFrame()
			...
			if ((mouseX >= framePosition.X) and (mouseX <= (framePosition.X + frameSize.X)) and (mouseY >= framePosition.Y) and (mouseY <= (framePosition.Y + frameSize.Y))) then
				return true;
			else
				return false;
			end
		...
		inputService.InputChanged:Connect(function(input)
			if ((input == dragInput) and dragging) then
				...
				dragObject:TweenPosition(UDim2New(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale, yPos), "Out", "Quad", 0.15, true);
			end
		end);
		do
			do
			...
			do
				UserInterface.GetConfig = function(self)
					local ConfigTable = {};
				...
				UserInterface.LoadConfig = function(self, config)
			...
			do
				UserInterface.RemovePopups = function(self)
					if UserInterface.Popup then
						UserInterface.Popup:Remove();
						UserInterface.Popup = nil;
					end
				end;
				UserInterface.NewPopup = function(self, Configuration)
					UserInterface.Popup = {Remove=Configuration.Remove,ID=Configuration.ID};
				end;
			end
			do
				UserInterface.Watermark = function(self, text)
					...
					WatermarkTitle.TextSize = 13;
					WatermarkTitle.RichText = true;
					local WatermarkOptions = {};
					WatermarkOptions.ChangeText = function(self, newtext)
					...
					WatermarkOptions.SetVisibility = function(self, visibility)
						Watermark.Visible = visibility;
					end;
					WatermarkOptions.SetPosition = function(self, v2Pos)
...
local nigga = (function()
	...
	loaderOptions.new = function(self)
		...
		for i, v in pairs(Loader:GetDescendants()) do
			...
			if hasProperty(v, "ImageTransparency") then
		...
		local names_to_look_and_ignore = {"Information","MainTitle","InfoTitle","OptionsTitle","Shadow1"};
		loaderOptions.FadeIn = function(self)
			for i, v in pairs(Loader:GetDescendants()) do
				if (hasProperty(v, "BackgroundTransparency") and not table.find(names_to_look_and_ignore, v.Name)) then
					Tween(v, TweenInfo.new(0.2), {BackgroundTransparency=0});
				end
				if hasProperty(v, "TextTransparency") then
					Tween(v, TweenInfo.new(0.2), {TextTransparency=0});
				end
				if hasProperty(v, "ImageTransparency") then
					Tween(v, TweenInfo.new(0.2), {ImageTransparency=0});
				end
				if (hasProperty(v, "Transparency") and v:IsA("UIStroke")) then
					Tween(v, TweenInfo.new(0.2), {Transparency=0});
				end
			end
		end;
		loaderOptions.FadeOut = function(self)
			for i, v in pairs(Loader:GetDescendants()) do
				if hasProperty(v, "BackgroundTransparency") then
					Tween(v, TweenInfo.new(0.2), {BackgroundTransparency=1});
				end
				if hasProperty(v, "TextTransparency") then
					Tween(v, TweenInfo.new(0.2), {TextTransparency=1});
				end
				if hasProperty(v, "ImageTransparency") then
					Tween(v, TweenInfo.new(0.2), {ImageTransparency=1});
				end
				if hasProperty(v, "Transparency") then
					Tween(v, TweenInfo.new(0.2), {Transparency=1});
				end
			end
		end;
		loaderOptions.ChangeInfoText = function(self, newInfoText)
			Information.Text = newInfoText;
		end;
		loaderOptions.Load = function(self)
	...
	loaderOptions.on_completed = function(script)
		loaderOptions.Completed.Event:Connect(script);
	end;
	loaderOptions.on_auto_load_stop = function(script)
		loaderOptions.AutoLoadStop.Event:Connect(script);
	end;
	loaderOptions:new();
	local info_text_table = {("Welcome " .. LocalPlayer.DisplayName),"This Game is not Supported","Loaded Universal Script - 1.0"};
	...
	local function my_script()
		if true then
			(function()
				...
				AuxScan.NewFunction = function(self, Name, Func)
					AuxScan.Objects[Name] = Func;
				end;
				AuxScan.Run = function(self)
				...
				AuxScan.ChangeText = function(self, TextObject, FuncName)
					TextObject.Text = FuncName;
				end;
				local da_hood = {functions={}};
				...
				local aimassist = {is_firing=false,enabled=false,keybind=false,smoothness={smoothing_start=0,smoothing_end=0,smoothing_boost=0},fieldofview=100,closest_mode="none",bones=nil,character=nil,lock_target=false,configuration={visibility=false,wallcheck=false,team=false,friends=false},jitter={x=0,y=0,z=0},use_mouse_sensitivity=false,use_camera=true,lock_target_state=false,prediction=0,smoothingtype="Linear",custom_calculation=false,auto_prediction=false,enable_on_move=false};
				local silentaim = {enabled=false,keybind=false,smoothness=1000,fieldofview=100,closest_mode="none",bones=nil,character=nil,lock_target=false,configuration={visibility=false,wallcheck=false,team=false,friends=false},lock_target_state=false,prediction=0,smoothingtype="Linear",custom_calculation=false,auto_prediction=false,position=nil,look_at=false,sync_with_aim_assist=false,update_time=60,old_frame=tick(),fov_calculation="pixels"};
				local Characters = {Prediction={JumpTimes={},LandTimes={},AirTimes={},VerticalPositions={},VerticalVelocity={},AirPrediction={},PeakTimes={},OldHorizontalVelocity={}}};
				local current_mouse_arg = "UpdateMousePosI";
				local mouse_arg = {[2788229376]=(current_mouse_arg or nil),[16033173781]=(current_mouse_arg or nil),[17895097441]=("UpdateMousePosition" or nil),[17895262040]=("UpdateMousePosition" or nil),[9825515356]=("MousePosUpdate" or nil)};
				local bullet_support = {[4312377180]={path=((workspace:FindFirstChild("MAP") and workspace.MAP:FindFirstChild("Ignored")) or nil)},[1008451066]={path=((workspace:FindFirstChild("Ignored") and workspace.Ignored:FindFirstChild("Siren") and workspace.Ignored.Siren:FindFirstChild("Radius")) or nil)},[6133219615]={path=((workspace:FindFirstChild("Ignored") and workspace.Ignored:FindFirstChild("Siren") and workspace.Ignored.Siren:FindFirstChild("Radius")) or nil)},[3985694250]={path=((workspace and workspace:FindFirstChild("Ignored")) or nil)}};
				...
				if please_load then
					do
						(function()
							da_hood.functions.is_alive = function(self, player)
								return (player and player.Character and player.Character:FindFirstChildOfClass("Humanoid") and (player.Character:FindFirstChildOfClass("Humanoid").Health > 0) and true) or false;
							end;
							da_hood.functions.is_localplayer_alive = function(self)
								return self:is_alive(LocalPlayer);
							end;
							da_hood.functions.is_moving = function(self, player)
								if player then
							...
							da_hood.functions.is_localplayer_moving = function(self)
								return self:is_moving(LocalPlayer);
							end;
							da_hood.functions.wall_check = function(self, Character)
							...
							da_hood.functions.getentities = function(self)
								local entities = {};
								for _, player in ipairs(playerService:GetPlayers()) do
									if (player == LocalPlayer) then
										continue;
							...
							da_hood.functions.getnearestplayertomouse = function(self, distance, settings)
								local entity = nil;
							...
							da_hood.functions.custom_getnearestplayertomouse = function(self, distance, settings)
								local entity = nil;
								local distance = distance;
								for i, v in next, self:getentities() do
									if (settings.friends and playerService[v.Name]:IsFriendsWith(LocalPlayer.UserId)) then
							...
							da_hood.functions.getclosestpart = function(self, character, config)
							...
							da_hood.functions.closestpoint = function(self, part)
								...
								local Origin = ((MouseRay.Y >= (part.Position - (part.Size / 2)).Y) and (MouseRay.Y <= (part.Position + (part.Size / 2)).Y) and (part.Position + Vector3New(0, -part.Position.Y + MouseRay.Y, 0))) or part.Position;
							...
							da_hood.functions.mouse_aim = function(self, arguments)
							...
							da_hood.functions.custom_prediction = function(self, HitPosition, Vel, BulletTravel, Configuration)
							...
							da_hood.functions.camera_aim = function(self, arguments)
								if (not type(arguments) == "table") then
									return;
								end
								local smoothness = arguments.smoothness;
								...
								if custom_calculation then
									...
									local aimpos = self:custom_prediction(arguments.position, player_velocity, ping, {Character=character,Prediction=prediction_value,AutoPrediction=auto_prediction,Resolver=false,ResolverValue=nil});
							...
							da_hood.functions.get_main_event = function(self)
							...
							da_hood.functions.get_gun = function(self)
								local gun;
							...
							da_hood.functions.get_gun_2 = function(self, player)
								local info = {isgunequipped=false,ammo=nil,tool=nil};
							...
							da_hood.functions.get_gun_from_character = function(self, character)
								local gun;
							...
							mouse_arg.Get = function(self)
								if not mouse_arg[game.PlaceId] then
									return "UpdateMousePos";
								end
								return mouse_arg[game.PlaceId];
							end;
							da_hood.functions.get_aim_arg = function(self)
								return mouse_arg:Get();
							end;
						end)();
					end
					local Interface = UserInterface({title='    Nixius<font color="rgb(167, 140, 255)">.xyz</font>'});
					do
						do
							...
							local changed = {start=Instance.new("BindableEvent"),end_value=Instance.new("BindableEvent"),boost_value=Instance.new("BindableEvent")};
							task.spawn(function()
								while task.wait() do
									...
									local tweeninfo = TweenInfo.new(boost_value, Enum.EasingStyle[smoothing_animation], Enum.EasingDirection.Out);
							...
							aimassist.get_position = function(self)
								local position;
							...
							aimassist.aim = function(self)
								da_hood.functions:camera_aim({smoothness=current_smoothing,use_mouse_sensitivity=aimassist.use_mouse_sensitivity,jitter=aimassist.jitter,position=aimassist:get_position(),use_camera=aimassist.use_camera,prediction=aimassist.prediction,character=aimassist.character,smoothingtype=aimassist.smoothingtype,custom_calculation=aimassist.custom_calculation,auto_prediction=aimassist.auto_prediction});
							end;
							aimassist.enable = function(self)
								if ((tick() - old_frame) >= (1 / update_time)) then
									...
									if aimassist.enable_on_move then
										if not da_hood.functions:is_localplayer_moving() then
											return;
										end
										aimassist:aim();
									else
							...
							local gacHandlers = {};
							gacHandlers.stop = function()
								run_gac = false;
							end;
							gacHandlers.resume = function()
								run_gac = true;
								coroutine.wrap(g_a_c)();
							end;
						...
						do
							...
							notification.new_text = function(self, text, time)
								local NotificationTable = {Text=text,Time=(os.clock() + time),Lerp=2,Offset=2,Object=DrawingClass({"Text",{Visible=false,Font=2,Size=16,Outline=true,Color=Color3.fromRGB(255, 255, 255)}})};
						...
						do
							local config = {path="nixius.xyz/configs/tycoonrng/",file="",config_name=""};
							config.get_list = function(self)
								local config_list = {};
							...
							profiles:Button({title="Load Config",callback=function()
								...
								if not isfile(config.path .. config.file .. ".txt") then
									return;
								end
								loading_config = true;
							...
							getgenv().theme = {accent=Color3FromRGB(167, 140, 255),scroll=Color3FromRGB(167, 140, 255)};
